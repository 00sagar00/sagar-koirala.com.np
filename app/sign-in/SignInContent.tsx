...
import { getStoredUser, setSessionEmail } from "../auth-storage";
...
export default function SignInContent() {
  ...
  const isRegisteredMessage = searchParams.get("registered") === "1";
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (normalizedEmail !== storedUser.email || password !== storedUser.password) {
    ...
    <main className="mx-auto flex min-h-screen w-full max-w-6xl items-center justify-center px-6 py-12 md:px-10">
      <section className="w-full max-w-md rounded-2xl border border-emerald-200 bg-emerald-50/40 p-6 shadow-sm dark:border-emerald-900 dark:bg-emerald-950/30">
        ...
        <div className="mt-5 flex items-center justify-between text-sm">
          ...
          <Link href="/sign-up" className="font-semibold text-emerald-700 hover:text-emerald-900 dark:text-emerald-300 dark:hover:text-emerald-100">
            Create account
          </Link>
        </div>
      </section>
